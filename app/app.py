...
def render_dataset_statistics(df, api_key):
    ...
    with tab1:
        # 1. User Metrics
        num_users = df["Subject ID"].nunique()
        total_posts = len(df)
        ...
        if "Prob_Severe_Depressed" in df.columns and "Prob_Moderate_Depressed" in df.columns:
            avg_severe = df["Prob_Severe_Depressed"].mean()
            avg_moderate = df["Prob_Moderate_Depressed"].mean()
        ...
        if 'Date' in df.columns:
            # Resample by Month
            activity_df = df.set_index('Date').resample('M').size().reset_index(name='Posts')
            fig_time = go.Figure(data=[go.Bar(x=activity_df['Date'], y=activity_df['Posts'], name='Posts')])
    ...
    with tab2:
        ...
        with col_stats_1:
             ...
             top_activity = df["Subject ID"].value_counts().head(10).reset_index()
             top_activity.columns = ["User ID", "Post Count"]
             st.table(top_activity.set_index("User ID"))
        ...
        with col_stats_2:
             ...
             def perform_calculation():
                 with st.spinner("Calculating risk scores for all users... (This is processed once and cached)"):
                     users = df["Subject ID"].unique()
                     risk_list = []
                     ...
                     for i, uid in enumerate(users):
                         u_data = df[df["Subject ID"] == uid].copy()
                         if u_data.empty: continue
                             
                         # Calculate EMA (using default 15 days for global comparison)
                         r_series = ema.calculate_risk_score(u_data, half_life=15)
                         
                         if not r_series.empty:
                                 curr_risk = r_series.iloc[-1]
                                 avg_risk = r_series.mean()
                                 max_risk = r_series.max()
                         ...
                         if i % 10 == 0:
                              progress_bar.progress((i + 1) / total_u)
                     
                     progress_bar.progress(1.0)
                     
                     if risk_list:
                         r_df = pd.DataFrame(risk_list)
                         # Sort by Current Risk
                         r_df = r_df.sort_values("Current Risk", ascending=False).head(20) # Keep top 20 in cache
                         
                         # Save to disk
                         r_df.to_json(cache_path)
                         return r_df
                     return None
